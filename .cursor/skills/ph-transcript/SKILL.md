---
name: ph-transcript
description: Use for Predictive History transcript work, including YouTube caption capture, imported source text, ASR audit, verbatim transcript sync, curated lecture transcript repair, exact-body-match checks, and quote-grade verification boundaries. Activate on "ph transcript", "youtube transcript", "predictive history transcript", or transcript fidelity tasks; do not use for strategy-codex raw-input ingestion.
---
...
Before fetching, syncing, importing, or editing transcript material, inspect the checkout for the active transcript stack:
...
- Curated lecture layer: `lectures/*.md`, book `*-transcript.md` files, and especially `## Full transcript`.
...
Record source tier, quality score, content hash, and status in the manifest where tooling supports those fields. Respect `TRANSCRIPT_MIN_QUALITY`; do not silently accept low-quality transcripts. Use `--keep-low-quality` only when retaining suspect material is intentional.

For single-video work, fetch or refresh by URL or video ID. For imported source text, preserve provenance and do exact-body-match checks before claiming transfer fidelity. Resume without overwriting existing raw transcript files. Use `--force` only when explicit.
...
Files under `verbatim-transcripts/*.md` are generated and overwrite-prone. Curated `lectures/*.md` and book transcript edits are separate layers.

## Exact-Body-Match Rules

When transcript text is imported from another repository, lecture file, generated verbatim source, or transfer authority, compare the source body against the destination body with `scripts/validate-transcript-fidelity.ps1` or the relevant spine validator. Do not silently normalize, paraphrase, or repair quoted transcript bodies while claiming exact transfer.
...
After changing transcript tooling or transcript material, run the relevant repo checks:

- `.\scripts\validate-ph-skills.ps1`
- `.\scripts\validate-transcript-skill.ps1`
- `.\scripts\validate-transcript-substrate.ps1`
- `.\scripts\validate-all.ps1`
- `python -m pytest tests/test_normalize_lecture_transcript_asr.py tests/test_youtube_transcripts.py -q`
