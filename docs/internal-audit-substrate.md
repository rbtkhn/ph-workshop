...
This internal substrate helps maintainers and agents inspect the completed Civilization spine and civ-ph layer without changing transcript or reader-facing prose.
...
- `.\scripts\audit-civ-ph.ps1`
- `.\scripts\audit-review-queue.ps1`

`validate-all.ps1` is the default maintainer command. It runs the Civilization spine, Great Books spine, civ-ph, and orientation validators, regenerates tracked reports, scans public surfaces, checks stale placeholder-like language, and runs `git diff --check`.
...
- Public civ-ph entries remain the reader-facing orientation layer.
