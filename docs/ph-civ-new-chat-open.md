...
2. Read `llms.txt`.
3. Run `scripts/ph-civ-open.ps1`.
4. If a local `ph-civ` checkout is available, run `scripts/ph-civ-open.ps1 -PhCivPath <path>`.
5. Summarize the gate state before proposing work.
