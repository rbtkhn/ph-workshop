...
if (-not $entryFiles) {
  throw "No civ-ph entry files found in $CorpusPath"
}
...
foreach ($file in $entryFiles) {
  ...
  if (-not $frontmatter) {
    throw "civ-ph entry $($file.FullName) is missing frontmatter"
  }

  foreach ($field in $requiredFields) {
    if (-not (Get-FrontmatterValue -Frontmatter $frontmatter -Key $field)) {
      throw "civ-ph entry $($file.FullName) is missing frontmatter field '$field'"
    }
  }

  if ((Get-FrontmatterValue -Frontmatter $frontmatter -Key 'derived_corpus') -ne 'civ-ph') {
    throw "civ-ph entry $($file.FullName) must set derived_corpus: civ-ph"
  }
  ...
  if ($reviewStatus -notin @('calibration_seed', 'in_review', 'draft_pending_analysis')) {
    throw "civ-ph entry $($file.FullName) has invalid review_status '$reviewStatus'"
  }

  $sourceId = Get-FrontmatterValue -Frontmatter $frontmatter -Key 'source_id'
  if ($file.Name -ne "$sourceId.md") {
    throw "civ-ph entry filename $($file.Name) must match source_id '$sourceId'"
  }
  ...
  if ($placementWeight -notin @('strong', 'medium', 'light')) {
    throw "civ-ph entry $($file.FullName) has invalid placement_weight '$placementWeight'"
  }

  foreach ($section in $requiredSections) {
    if ($text -notmatch "(?m)^##\s+$([regex]::Escape($section))\s*$") {
      throw "civ-ph entry $($file.FullName) is missing section '$section'"
    }
  }
  ...
  foreach ($pathField in @('source_corpus_path', 'source_chapter_path', 'commentary_path')) {
    ...
    if (-not (Test-Path -LiteralPath $resolvedTarget -PathType Leaf)) {
      throw "civ-ph entry $($file.FullName) points $pathField to missing file: $target"
    }
  }
}
...
$manifestMatches = [regex]::Matches($manifestText, '(?m)^\s+civ_ph_path:\s*(\S+)\s*$')
if ($manifestMatches.Count -eq 0) {
  throw "Manifest $ManifestPath does not declare any civ_ph_path values"
}
...
foreach ($match in $manifestMatches) {
  ...
  $resolvedTarget = Resolve-RepoPath -Path $target
  if (-not (Test-Path -LiteralPath $resolvedTarget -PathType Leaf)) {
    throw "Manifest civ_ph_path points to missing file: $target"
  }
}
...
foreach ($file in $entryFiles) {
  ...
  if ($manifestText -notmatch [regex]::Escape($relativePath)) {
    throw "civ-ph entry $relativePath is not referenced by civ_ph_path in $ManifestPath"
  }
}
