...
foreach ($file in $entryFiles) {
  ...
  if ($manifestText -notmatch [regex]::Escape($relativePath)) {
    $warnings.Add("not referenced by ph_civ_path in manifest") | Out-Null
  }
...
if ($hardFailures.Count -gt 0) {
  throw "PH-CIV audit hard failures: $($hardFailures -join '; ')"
}
...
$summary | ConvertTo-Json -Depth 8 | Set-Content -LiteralPath (Join-Path $resolvedReportsDir 'ph-civ-health.json') -Encoding utf8

$md = New-Object System.Collections.Generic.List[string]
$md.Add('# PH-CIV Health') | Out-Null
$md.Add('') | Out-Null
$md.Add('Internal audit snapshot for PH-CIV orientation entries.') | Out-Null
$md.Add('') | Out-Null
...
$md | Set-Content -LiteralPath (Join-Path $resolvedReportsDir 'ph-civ-health.md') -Encoding utf8

[pscustomobject]@{
  Report = 'ph-civ-health'
